...
execute as @a if score @s s_id = @e[tag=sheep_temp,limit=1] s_thrower_id at @s anchored eyes run tp @e[tag=sheep_temp,scores={s_ulti_sheep=..100},limit=1] ^ ^0.3 ^2 ~ ~
execute as @a if score @s s_id = @e[tag=sheep_temp,limit=1] s_thrower_id at @s run tp @e[tag=sheep_temp,scores={s_ulti_sheep=101..},limit=1] ~ ~3 ~ ~ ~
