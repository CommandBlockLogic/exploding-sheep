...
# check confirm
execute if score game_status system matches 0 if score system_ani_confirm system matches 0 run tellraw @a [">> 即将播放游戏背景动画，15秒内再次按下",{"text":"按钮","underlined":true,"clickEvent":{"action":"run_command","value":"/trigger tellraw_anime set 1"}},"开始播放！"]
# start game
