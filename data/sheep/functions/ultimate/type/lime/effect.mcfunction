...
execute if entity @s[tag=sheep_shot_by_launcher] run scoreboard players add @a[distance=..8] s_energy 50

execute if entity @s[tag=!sheep_team_blue,tag=!sheep_team_red,tag=!sheep_shot_by_launcher] if entity @a[distance=..8] run tag @s add sheep_temp
execute if entity @s[tag=!sheep_team_blue,tag=!sheep_team_red,tag=!sheep_shot_by_launcher] as @a[distance=..8] if score @s s_id = @e[tag=sheep_temp,limit=1] s_thrower_id run tag @s add player_temp
