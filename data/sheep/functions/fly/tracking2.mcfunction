...
execute at @s[scores={s_flying_time=20..}] positioned ^ ^ ^0.35 if entity @a[gamemode=!spectator,distance=..1] run tag @s add sheep_collision
execute at @s[scores={s_flying_time=20..}] positioned ^ ^ ^0.35 if entity @e[tag=monster,distance=..1] run tag @s add sheep_collision
