...
# check confirm
execute if score game_status system matches 0 if score system_flag system matches 1 if score player_diff system matches -1..1 if score system_dm_confirm system matches 0 run tellraw @a [">> 即将开始部落死亡竞赛模式, 15秒内再次按下按钮开始游戏!\n - 耗尽对方生命值以取得胜利\n - 别忘记选择顺手的羊羊类型！"]
# start game
