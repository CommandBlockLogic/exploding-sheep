...
# hint
execute if score game_status system matches 1.. if score @s system_temp1 matches 0 run tellraw @s [">> 您已充满羊羊能量! 跳上传送石回到战场!"]
execute if score @s system_temp1 matches 1.. run tellraw @s [">> 等待",{"score":{"name":"*","objective":"system_temp1"}},"秒补充羊羊能量后, 跳上传送石回到战场!"]