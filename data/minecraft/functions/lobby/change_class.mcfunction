...
execute at @e[tag=system_class_mark_pink] as @a[distance=..0.3] unless score @s s_class matches 6 if score @s s_class matches ..100 run tellraw @s [">> 切换至 §d治愈§r 系\n    羊羊在附近生成治愈阵, 其中的队友会受到治愈","\n    手持小羊大炮按F即可切换!"]
execute at @e[tag=system_class_mark_pink] as @a[distance=..0.3] unless score @s s_class matches 6 if score @s s_class matches ..100 run scoreboard players set @s s_class 6
